...
export function SalesByBarcode({ role, store }: SalesByBarcodeProps) {
  ...
  const initialStore = store && store !== "Все магазины" && store !== "all" ? store : "Магазин 1";
...
const discountedCartTotal = useMemo(() => {
  ...
  items: cart.map((item) => {
    ...
    return {
      ...
      const receiptSnapshot = {
        ...
        store: cart[0]?.store || effectiveStore,
        cashier: role === "cashier" ? "Кассир" : "Администратор",
  ...
  >
    {allowedStores.map((option) => (
      ...
      <div className="grid w-full max-w-full min-w-0 grid-cols-1 gap-6 2xl:grid-cols-[minmax(0,1fr)_minmax(360px,520px)]">
        <div className="w-full max-w-full min-w-0 rounded-3xl border border-white/10 bg-white/[0.04] p-6">
          <div className="text-xs uppercase tracking-[0.3em] text-zinc-500">
            Добавить в чек
          </div>
            ...
            <table className="min-w-full border-collapse text-left text-sm">
              ...
              <tbody>
                {cart.map((item) => (
                  ...
                  >
                    <td className="max-w-[180px] break-words whitespace-normal px-4 py-4 text-white">{item.name}</td>
                    <td className="break-all px-4 py-4 text-zinc-300">
                      {item.barcode}
                    </td>
  ...
  <div className="mt-4 overflow-x-auto rounded-2xl border border-white/10">
    <table className="min-w-full border-collapse text-left text-sm">
      ...
      <InternalSaleReceiptModal
        ...
        cashier={receiptSale?.cashier || (role === "cashier" ? "Кассир" : "Администратор")}
