      ...
      for (const payload of payloads) {
        ...
        if (response.ok) {
          ...
          const receiptSnapshot = {
            ...
            store: items[0]?.store || currentStore,
            cashier: user?.displayName || user?.name || user?.username || "Кассир",
  ...
  return (
    <main className="min-h-screen bg-black px-4 pb-56 pt-5 text-white">
      ...
      <InternalSaleReceiptModal
        ...
        cashier={receiptSale?.cashier || user?.displayName || user?.name || user?.username || "Кассир"}
